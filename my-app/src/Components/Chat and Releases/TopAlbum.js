...
const Chart = ({ data, selectAlbum }) => {
  ...
  return (
    <section className="h-full">
      ...
      {data.length > 0 ? (
        <div className="overflow-x-scroll overflow-y-hidden scrollbar-none mt-8 md:mt-2 lg:overflow-x-hidden lg:ml-0 lg:overflow-y-scroll lg:h-[85%]">
          <div className="lg:hidden">
            <Swiper
              className="w-full min-w-fit  lg:px-2 h-[250px] min-h-fit"
              freeMode={true}
              breakpoints={{
                0: {
                  slidesPerView: 1.3,
                ...
                return (
                  <SwiperSlide
                    ...
                    onClick={() => {
                      selectAlbum(chart);
                      Navigate(
                        `/topalbums/album?title=${chart.title}&id=${chart.id}`
                      );
                    }}
                  >
                    <Text content={chart} />
